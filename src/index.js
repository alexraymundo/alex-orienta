...
async function api(req, env, url) {
  ...
  if (path === "/api/client/ai" && req.method === "POST") {
    ...
    if (!access.consent_confirmed) {
      return json({
        error: "Reflexión AI todavía no está habilitada porque falta confirmar el consentimiento."
      }, 403);
    }

    if (!access.ai_enabled) {
      return json({
        error: "Alex todavía no ha habilitado Reflexión AI para este proceso."
      }, 403);
    }

    if (!env.AI || typeof env.AI.run !== "function") {
      return json({
        error: "Reflexión AI no está disponible en este momento."
      }, 503);
    }
    ...
    const system = `
Eres "Reflexión AI", una herramienta complementaria dentro de Alex Orienta.
...
`.trim();
    ...
    try {
      ...
      if (!reply) {
        return json({
          error: "Reflexión AI no pudo generar texto en este momento."
        }, 502);
      }
      ...
      return json({
        error: `Reflexión AI: ${safeText(error?.message || error, 500)}`
      }, 502);
    }
  }
  ...
  if (path === "/api/admin/ai-reflection" && req.method === "POST") {
    ...
    if (!message) {
      return json({
        error: "Escribe un mensaje para probar Reflexión AI."
      }, 400);
    }
